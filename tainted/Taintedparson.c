...
#define parson_tainted_free(t, p)   (t_free<t>((t *)p))
...
_Callback static char *    process_string(const char * input , size_t len);

 char * parson_string_tainted_malloc(size_t sz)  {
    if(sz >= SIZE_MAX)
        return NULL;
    char * p  = (char *)t_malloc<char>(sz + 1);
    if (p != NULL)
    p[sz] = 0;
    return (char *)p;
}
...
if (length > 2 && !t_strncmp((const char *)string, "-0", 2) && string[2] != '.') {
return 0;
}
while (length--) {
...
t_memcpy((void *)temp_names, object->names, object->count * sizeof(char*));
t_memcpy((void *)temp_values, object->values, object->count * sizeof(TJSON_Value*));
}
...
name_length = t_strlen(((const char *)object ->names [i ]));
if (name_length != name_len) {
continue;
}
if (t_strncmp((const char *)object ->names [i ],
...
const char * str_cpy = (const char *)parson_string_tainted_malloc
(str_len*sizeof(char));
...
char * (*process_string)(const char * input,size_t len)) {
int str_len = t_strlen(string);
const char * string_start = parson_string_tainted_malloc(str_len*sizeof(char));
...
char * new_string = get_quoted_string(str_cpy, process_string);
if (new_string == NULL) {
t_strcpy(string, str_cpy);
return NULL;
}
...
return NULL;
  ...
  TJSON_Value * parse_number_value(const char * string) {
    ...
    char * end = NULL;
    double number = 0;
    number = t_strtod((char *)str_cpy, (char * *)&end);
    if (!is_decimal((char *)str_cpy, (size_t)(t_strlen(string) - t_strlen(end)))) {
        t_strcpy(string, str_cpy);
        return NULL;
    }
...
const char * dot_position = t_strchr(name, '.');
if (!dot_position)  {
