...
if (temp_values == NULL) {
parson_free_unchecked(void, temp_names);
return JSONFailure;
}
...
int unprocessed_len = strlen(unprocessed);
char* processed_ptr = (char*)malloc(processed_len*sizeof(char));
strcpy(processed_ptr, (const char *)processed);
char* unprocessed_ptr = (char*)malloc(unprocessed_len*sizeof(char));
...
string_len = strlen(string_start) - strlen(string) - 2; /* length without quotes */
...
return value;
 ...
 TJSON_Value * parse_boolean_value(const char * string) {
	int str_len = strlen(string);
	const char * str_cpy = (const char *)parson_string_tainted_malloc(str_len*sizeof(char));
	strcpy(str_cpy, string);
	size_t true_token_size = SIZEOF_TOKEN("true");
	size_t false_token_size = SIZEOF_TOKEN("false");
	if (strncmp("true", str_cpy, true_token_size) == 0) {
		str_cpy += true_token_size;
		strcpy(string, str_cpy);
		return json_value_init_boolean(1);
	} else if (strncmp("false", str_cpy, false_token_size) == 0) {
  ...
  TJSON_Value * parse_number_value(const char * string) {
    ...
    *end = NULL;
    double number = 0;
    number = strtod((char *)str_cpy, end);
    if (!is_decimal(str_cpy, (size_t)(strlen(string) - strlen((const char *)*end)))) {
        strcpy(string, str_cpy);
        return NULL;
    }
